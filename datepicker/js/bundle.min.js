...
(function() {
  ...
  users = angular.module('datepicker', []);
...
(function() {
  ...
  mainController = function($scope, $filter) {
    var createMonthArr, setCurrentMonthPrams, setSelectedDateText, vm;
    vm = this;
    setCurrentMonthPrams = function() {
      ...
      return vm.monthLength = new Date(vm.year, vm.currentMonth + 1, 0).getDate();
    };
    setSelectedDateText = function() {
      vm.selectedDateTextFormat1 = $filter('date')(vm.selectedDate, 'd / ') + vm.months[vm.selectedDate.getMonth()][1] + $filter('date')(vm.selectedDate, ' / yyyy');
      return vm.selectedDateTextFormat2 = $filter('date')(vm.selectedDate, 'd ') + vm.months[vm.selectedDate.getMonth()][1] + $filter('date')(vm.selectedDate, ' yyyy ');
    };
    createMonthArr = function() {
      var currentDay, date, i, iterations, newWeek, ref, week;
      week = [];
      ...
      if (vm.currentMonth === 0 && currentDay === 0) {
        vm.firstDayOfMonth = currentDay = 6;
      }
      for (date = i = 1, ref = vm.monthLength; 1 <= ref ? i <= ref : i >= ref; date = 1 <= ref ? ++i : --i) {
        if (date !== 1) {
          week.push(date);
          ...
          if (currentDay === 6) {
            ...
            week[vm.firstDayOfMonth - 1] = 1;
          }
        }
      }
      return console.log("vm.weeks ", vm.weeks);
    };
    ...
    vm.months = [['Январь', 'Января'], ['Февраль', 'Февраля'], ['Март', 'Марта'], ['Апрель', 'Апреля'], ['Май', 'Мая'], ['Июнь', 'Июня'], ['Июль', 'Июля'], ['Август', 'Августа'], ['Сентябрь', 'Сентября'], ['Октябрь', 'Октября'], ['Ноябрь', 'Ноября'], ['Декабрь', 'Декабря']];
    setSelectedDateText();
    ...
    vm.changeMonth = function(operator) {
      ...
      setCurrentMonthPrams();
      return createMonthArr();
    };
    vm.selectDate = function(e, date) {
      if (!date) {
        return;
      }
      vm.selectedDate = new Date(vm.currentYear, vm.currentMonth, date, vm.dateNow.getHours(), vm.dateNow.getMinutes());
      setSelectedDateText();
      angular.element(document.querySelectorAll('.selected-date')).removeClass('selected-date');
      return angular.element(e.target).addClass('selected-date');
    };
    vm.showSelectedDate = (function(_this) {
      return function(date) {
        return vm.selectedDate.getMonth() === vm.currentMonth && vm.selectedDate.getFullYear() === vm.currentYear && vm.selectedDate.getDate() === date;
      };
    })(this);
  ...
  angular.module('datepicker').controller('mainController', mainController);
